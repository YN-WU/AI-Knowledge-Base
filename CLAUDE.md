...
- **個人化**（2026-06 新增）：AI 記住使用者偏好、跨對話累積脈絡、客製化建議。例如 ChatGPT Memory、Claude Projects、個人 AI 代理（Gemini Spark / Hermes Agent）、profile-based 助理。只要文章核心是「越用越懂你」就放。
- **程式開發**（2026-06 從「程式設計」rename）：涵蓋寫程式 + 跑 local model + devops + 開發環境配置 + 開發者工具導入，不只「寫 code」這個動作。
- **觀念學習**：給「不能直接用、但讀者該知道」的資訊型文章（法律新聞、產業趨勢、AI 倫理）。
- **法律規範**：只用在**第一層** `tag`，不放進第二層 `tags`（避免跟「觀念學習」重複）。
...
1. **有相關就加，不限上限**（2026-06 拿掉原「1-4 個」上限）。但每個都要真的對應到原文，別為了多而湊；少有少的準確度、多有多的覆蓋度。

2. **嚴格對應原文明寫的應用場景，不要推斷**：原文哪句寫到什麼場景，tags 才放對應的那個。

3. **三個常見「不該推斷」的陷阱**：
   - **產品類型總稱**：「這是語音模型」→ 不自動 = 聲音處理 ❌
   - **產品屬性**：「這是 AI 代理人 / 生圖模型」→ 不自動 = 自動化 / 工作流整合 / 簡報設計 ❌
   - **Delivery channel**：「在 GitHub 開源 / 上架 Ollama / Hugging Face」→ 不自動 = 程式開發 ❌（那只是「在哪裡可以用」的發布位置，不是 work scenario）

---

**判斷灰色地帶：「產品總稱」vs「文章單獨點名的能力」**

「產品總稱」不能套對應 tag，但若文章**在內文裡單獨點名某項能力**作為賣點/解鎖點，那項能力對應的 tag **可以放**。
- 反例（總稱）：「Gemini Omni 是多模態模型」→ 不放 聲音處理
- 正例（單獨點名）：「Gemma 4 首次讓中型模型具備原生音訊輸入功能」→ 可放 聲音處理（這句明確強調音訊能力解鎖、是文章核心賣點之一）
- 差別在於：是只描述產品類型，還是內文裡實際強調這項能力對使用者的意義。

**How to apply**：新增條目時兩個欄位都要填。挑第二層 tag 時對照三大原則 + 灰色地帶準則；舊文章維持原狀不主動回補。
