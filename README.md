...
頁面有一個「篩選日期」欄位，填一個日期（例：`2026-05-08`），下方 4 個分類的篇數會**即時連動**顯示「該日（含）以後實際筆數」。按「產出」就抓那天以後的全部內容。適合「自上次寄出後新增的全部內容」這類場景。

**B. 篇數模式**

日期欄位**留空**就走篇數模式：每個分類取最新 N 篇。N 來自頁面初始化時自動偵測的篇數（目前 4 欄都是 readonly 顯示，不開放手動編輯，要調整輸出範圍請改篩選日期）。

### 自動偵測 + 連動
- **月份標題**：從最新一筆日期推算 `YYYY 年 M 月號`（首次填好就尊重手改）
- **4 個篇數欄位**（重點趨勢 / Prompt 分享 / 工具介紹 / 30 秒看趨勢）：頁面開啟時自動帶入；改篩選日期之後即時更新
- 4 個篇數顯示為「重點趨勢：3 篇」inline 格式，不再開放編輯

### 工作流程
1. 編輯 `data/articles.json` + `data/weekly-summaries.json` 等（首頁會即時更新）
2. 開 `#outlook-gen` → 填篩選日期（或留空走篇數模式）→ 確認下方 4 個篇數正確 → 按「產出」
3. 預覽 OK 後按「複製 HTML」
